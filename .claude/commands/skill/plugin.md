...
**Usage**:
- `/plugin <UI_LIBRARY_NAME>` - Full plugin development process
- `/plugin <UI_LIBRARY_NAME> --compatibility-only` - Compatibility analysis only

**Examples**:

- `/plugin "Chakra UI v2"` - Execute full plugin development process
- `/plugin "Chakra UI v2" --compatibility-only` - Execute compatibility analysis only
- `/plugin "Material-UI v5"` - Full development
- `/plugin "Ant Design v5" --compatibility-only` - Quick compatibility check
...
**Command**: `/plugin <UI_LIBRARY_NAME>`
...
**Command**: `/plugin <UI_LIBRARY_NAME> --compatibility-only`
...
/plugin "Radix UI" --compatibility-only
...
/plugin "Chakra UI" --compatibility-only
/plugin "Material-UI" --compatibility-only
/plugin "Ant Design" --compatibility-only
...
/plugin "Chakra UI v2" --compatibility-only
...
1. Quick Check Phase
   ↓
   /plugin "Library Name" --compatibility-only
   ↓
...
When you run `/plugin <UI_LIBRARY_NAME>`, Claude will:
...
```bash
# 정확한 라이브러리 이름과 버전 제공
/plugin "Material-UI v5" --compatibility-only
...
### 기본 사용법
```
/plugin "UI_LIBRARY_NAME"
```

### 호환성 체크만
```
/plugin "UI_LIBRARY_NAME" --compatibility-only
```
...
명령: /plugin "Radix UI" --compatibility-only
결과:
  📊 호환성 매트릭스
  - String Input: ✅ 100%
  - Select: ✅ 95%
...
결과:
  ...
  - 최종 선택: Material-UI
  → /plugin "Material-UI v5" (전체 개발 진행)
```
...
명령:
  1. /plugin "Radix UI" --compatibility-only
  2. (호환성 85% → 진행 결정)
  3. /plugin "Radix UI"
결과:
  - 완전한 프로젝트 구조 생성
...
💡 다음 단계:
- 호환성 ≥ 80% → /plugin "Chakra UI v2" (전체 개발 진행)
...
1. 정확한 라이브러리 이름과 버전 제공:
   /plugin "Chakra UI v2.8.0"
   /plugin "@chakra-ui/react v2"
