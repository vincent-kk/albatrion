...
일반적인 모달 시나리오에 대한 실용적인 사용 패턴입니다.
...
`subtype` prop은 컴포넌트 슬롯으로 전달되므로 시맨틱 타입별로 다른 스타일을 적용할 수 있습니다.
...
`ModalProvider`에서 전역으로, 또는 개별 모달 옵션에서 per-call로 컴포넌트 슬롯을 재정의할 수 있습니다.
...
`background.data`를 통해 `BackgroundComponent`에 타입이 지정된 데이터를 전달합니다.
...
`useModal`을 통해 열린 모달은 컴포넌트가 언마운트될 때 자동으로 닫힙니다. 폼이나 상세 뷰에 유용합니다.
...
커스텀 `ForegroundComponent`와 `useDestroyAfter`를 사용하여 `alert`로 토스트 메시지를 구현합니다.
...
`ModalProvider`의 `context` prop을 통해 모든 모달 컴포넌트 슬롯에 공유 데이터를 전달합니다.
