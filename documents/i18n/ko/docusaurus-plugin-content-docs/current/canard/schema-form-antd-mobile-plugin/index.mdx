...
JSON Schema를 설정하면 이 플러그인이 자동으로 터치 인터페이스에 최적화된 Ant Design Mobile 폼 컴포넌트를 렌더링합니다:
- **텍스트 필드** → Mobile `Input` component
- **숫자** → Mobile `Stepper` control
- **불리언** → `Checkbox` or `Switch`
- **열거형** → `Picker` or `CheckList`
- **날짜** → Mobile `DatePicker`, `CalendarPicker`

:::info 스크린샷
비주얼 데모 스크린샷이 `/img/demos/schema-form-antd-mobile-demo.png`에 추가될 예정입니다.
:::
