...
:::info `@slats/agents-assets-sync` 로 마이그레이션되었습니다

이 엔진은 [`@slats/agents-assets-sync`](../agents-assets-sync/) 로 마이그레이션되었으며,
새 엔진은 하나의 자산 트리를 Claude Code, Codex, 그리고 벤더 중립 `.agents` 레이아웃에
주입합니다. 이 이름으로 배포된 릴리스는 npm 에 그대로 남아 계속 동작하지만, 새로 배포되는
것은 없습니다 — 새 패키지는 `agents.assetPath` 를 선언하고 현재 엔진을 사용하세요.
아래 내용은 배포된 `claude.assetPath` 규약에 대한 문서입니다.
