...
Promise Modal provides a programmatic API for modal dialogs. Instead of managing modal state with React state variables, you call a function that returns a Promise:

1. **Call** `openModal(MyComponent, props)` from any code
2. **Modal appears** with your component rendered inside
3. **User interacts** and the modal resolves/rejects the Promise
4. **Result returned** as a resolved Promise value

:::info Screenshots
Visual demo screenshot will be added at `/img/demos/promise-modal-demo.png`.
:::
