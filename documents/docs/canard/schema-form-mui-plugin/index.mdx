...
When configured with a JSON Schema, this plugin renders Material UI form components following Material Design guidelines:
- **Text fields** → `TextField` with outlined/filled variants
- **Numbers** → `TextField` with number type
- **Booleans** → `Checkbox` or `Switch`
- **Enums** → `Select` or `RadioGroup`
- **Dates** → MUI `DatePicker`
- **Arrays** → Dynamic list with add/remove IconButtons

:::info Screenshots
Visual demo screenshot will be added at `/img/demos/schema-form-mui-demo.png`.
:::
