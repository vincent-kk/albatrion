...
`@canard/schema-form` ships coding-agent assets — the `schema-form-skill` expert skill, its knowledge base, and the `schema-form-rule` authoring rule — under `docs/agents/`. Each release publishes them alongside the package so AI assistants can reason about the form engine with package-specific context.

Inject those assets with `@slats/agents-assets-sync` (bin: `inject-agents-settings`). `--agent` decides where they land — `claude`, `codex`, or the vendor-neutral `agents`:

```bash
# Into ~/.claude (user-level)
npx -p @slats/agents-assets-sync inject-agents-settings --package=@canard/schema-form --agent=claude --scope=user

# Into the nearest existing .claude, walking up from cwd
npx -p @slats/agents-assets-sync inject-agents-settings --package=@canard/schema-form --agent=claude --scope=project

# The same assets into Codex locations (skills into .agents/, rules into AGENTS.md)
...
The CLI resolves `@canard/schema-form/package.json` via `createRequire`, reads its `agents.assetPath` (`docs/agents`), and applies hash-based diff into the selected agent's asset locations (skip unchanged, warn on divergence, require `--force` to overwrite).
