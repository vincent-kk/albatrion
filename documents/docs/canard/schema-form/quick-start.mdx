...
<LangTabs
  en={<>
    <p>
      Register plugins <strong>once</strong> at application entry point, before any
      <code>&lt;Form&gt;</code> renders. The call is idempotent — duplicate registrations
      are ignored by content hash.
    </p>
  </>}
  kr={<>
    <p>
      <code>&lt;Form&gt;</code>이 렌더링되기 전, 애플리케이션 진입점에서 <strong>한 번</strong> 플러그인을
...
<LangTabs
  en={<>
    <p>
      Use <code>Form.Group</code>, <code>Form.Label</code>, <code>Form.Input</code>,
      and <code>Form.Error</code> sub-components to build a custom layout.
      Each takes a <code>path</code> prop (JSONPointer string).
    </p>
  </>}
  kr={<>
    <p>
      <code>Form.Group</code>, <code>Form.Label</code>, <code>Form.Input</code>,
      <code>Form.Error</code> 서브 컴포넌트를 사용하여 커스텀 레이아웃을 구성합니다.
      각각 JSONPointer 문자열인 <code>path</code> prop을 받습니다.
    </p>
  </>}
/>
