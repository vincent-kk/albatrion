...
When configured with a JSON Schema, this plugin renders Ant Design Mobile form components optimized for touch interfaces:
- **Text fields** → Mobile `Input` component
- **Numbers** → Mobile `Stepper` control
- **Booleans** → `Checkbox` or `Switch`
- **Enums** → `Picker` or `CheckList`
- **Dates** → Mobile `DatePicker`, `CalendarPicker`

:::info Screenshots
Visual demo screenshot will be added at `/img/demos/schema-form-antd-mobile-demo.png`.
:::
