...
```
/hook    → useConstant, useTruthyConstant, useMemorize, useHandle, useSnapshot,
           useSnapshotReference, useReference, useVersion, useDebounce, useTimeout,
           useWindowSize, useEffectUntil, useLayoutEffectUntil, useOnMount,
           useOnMountLayout, useOnUnmount, useOnUnmountLayout, useRestProperties
/hoc     → withErrorBoundary, withErrorBoundaryForwardRef, withUploader
/portal  → Portal, Portal.Anchor, withPortal
/filter  → isReactComponent, isReactElement
/render  → renderComponent, remainOnlyReactComponent
```

### Usage patterns
- `useConstant(factory)` — initializes once, never re-creates
- `useHandle(fn)` — stable ref-backed callback (replaces useCallback with empty deps)
- `withErrorBoundary(Component, { fallback })` — wraps with error boundary
- Portal system requires `withPortal` HOC at tree root and `Portal.Anchor` to declare target
