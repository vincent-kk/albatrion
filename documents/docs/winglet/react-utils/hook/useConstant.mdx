...
```typescript
const useConstant: <Type>(input: Type) => Type
```
...
|------|------|-------------|
| `input` | `-` | The value to store as constant. If a function is passed, it's stored as-is, not executed |

## Returns

The constant value that remains unchanged throughout the component lifecycle
...
<WingletPlayground
  ...
  code={`// Stable object reference for React.memo optimization
const EmptyState = React.memo(({ config }) => { ... });
const Parent = () => {
  const defaultConfig = useConstant({ showIcon: true, message: 'No data' });
  return <EmptyState config={defaultConfig} />; // Never re-renders due to config
