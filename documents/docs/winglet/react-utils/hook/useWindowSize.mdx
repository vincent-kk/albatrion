...
An object containing the current window dimensions: - width: The inner width of the window in pixels (0 during SSR) - height: The inner height of the window in pixels (0 during SSR)
...
<WingletPlayground
  ...
  code={`// Basic responsive layout switching
const ResponsiveLayout = () => {
  const { width, height } = useWindowSize();
