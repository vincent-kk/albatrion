...
### Key exports
```
/scanner       → JsonSchemaScanner, SchemaVisitor, SchemaEntry, JsonScannerOptions
/async-scanner → JsonSchemaScannerAsync
/filter        → isObjectSchema, isArraySchema, isStringSchema, isNumberSchema,
                 isBooleanSchema, isNullSchema, isNonNullable*/isNullable* variants,
                 isCompatibleSchemaType, isIdenticalSchemaType
Main export    → resolveReference, InferValueType<Schema>
```

### Scanner usage
```ts
new JsonSchemaScanner({
  visitor: { enter(entry, ctx) {}, exit(entry, ctx) {} },
  maxDepth: 10,
  resolveReference: (ref, root) => resolvedSchema,
}).scan(rootSchema);
```

### Constraints
- Stack-based circular reference detection
- `$ref` resolution is opt-in via `resolveReference` option
- Async scanner returns `Promise<void>` from `scan()`
