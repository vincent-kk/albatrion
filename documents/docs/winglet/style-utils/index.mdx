...
`@winglet/style-utils` ships coding-agent assets — the `style-utils-skill` expert skill and its knowledge base covering `classnames`, CSS compression, scoped style management, and Shadow DOM usage — under `docs/agents/`. They are published alongside the package so AI assistants can reason about style injection with package-specific context.
...
The CLI resolves `@winglet/style-utils/package.json` via `createRequire`, reads its `agents.assetPath` (`docs/agents`), and applies hash-based diff into the selected agent's asset locations (skip unchanged, warn on divergence, require `--force` to overwrite).
