...
`@winglet/data-loader` ships coding-agent assets — the `data-loader-skill` expert skill and its knowledge base covering batching, caching, and advanced patterns — under `docs/agents/`. They are published alongside the package so AI assistants can reason about N+1 mitigation and scheduler semantics with package-specific context.
...
npx -p @slats/agents-assets-sync inject-agents-settings --package=@winglet/data-loader --agent=claude --scope=project
...
npx -p @slats/agents-assets-sync inject-agents-settings --package=@winglet/data-loader --agent=claude --scope=user --dry-run
npx -p @slats/agents-assets-sync inject-agents-settings --package=@winglet/data-loader --agent=claude --scope=user --force
```

The CLI resolves `@winglet/data-loader/package.json` via `createRequire`, reads its `agents.assetPath` (`docs/agents`), and applies hash-based diff into the selected agent's asset locations (skip unchanged, warn on divergence, require `--force` to overwrite).
