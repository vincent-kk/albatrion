...
**Purpose**: Request batching and caching for async data fetching. Solves N+1 problem.
...
new DataLoader<Key, Value, CacheKey = Key>(
  batchLoader: (keys: readonly Key[]) => Promise<(Value | Error)[]>,
...
loadMany(keys): Promise<(Value | Error)[]>
clear(key): this
clearAll(): this
prime(key, value): this
```

### Constraints
- `batchLoader` must return values in **same order** as input keys
- Failed keys should resolve to `Error` instances, not rejected promises
- Cache stores promises (not values) so concurrent loads share one request
