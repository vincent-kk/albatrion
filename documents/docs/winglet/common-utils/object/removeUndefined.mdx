...
```typescript
const removeUndefined: <Type>(input: Type) => Type
```
...
|------|------|-------------|
| `input` | `-` | Data structure to process (objects, arrays, or primitives) |

## Returns

New structure with undefined values removed, or original if no undefined found
...
<WingletPlayground
  ...
  code={`import { removeUndefined } from '@winglet/common-utils';
