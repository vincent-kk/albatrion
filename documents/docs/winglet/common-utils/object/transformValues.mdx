...
```typescript
const transformValues: <Type extends object, Key extends keyof Type, Value>(object: Type, getValue: (value: Type[Key], key: Key, object: Type) => Value) => Record<Key, Value>
```
...
|------|------|-------------|
| `object` | `-` | Object whose values will be transformed |
| `getValue` | `-` | Function that transforms each value, receives (value, key, object) |

## Returns

New object with original keys and transformed values
...
<WingletPlayground
  ...
  code={`import { transformValues } from '@winglet/common-utils';
