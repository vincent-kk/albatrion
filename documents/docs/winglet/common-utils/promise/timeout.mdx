...
```typescript
const timeout: (ms?: number, options?: DelayOptions) => Promise<never>
```
...
|------|------|-------------|
| `ms` | `-` | Timeout duration in milliseconds (default: 0) |
| `options` | `-` | Configuration options for timeout behavior |

## Returns

Promise that always rejects with TimeoutError after the specified delay
...
<WingletPlayground
  ...
  code={`import { timeout } from '@winglet/common-utils';
