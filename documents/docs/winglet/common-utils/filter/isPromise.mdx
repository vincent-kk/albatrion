...
```typescript
const isPromise: <T extends Promise<any>>(value: unknown) => value is T
```
...
|------|------|-------------|
| `value` | `-` | Value to test for Promise type |

## Returns

Type-safe boolean indicating whether the value is a Promise
...
<WingletPlayground
  ...
  code={`import { isPromise } from '@winglet/common-utils';
