...
```typescript
const isPlainObject: <T extends Record<PropertyKey, any>>(value: unknown) => value is T
```
...
|------|------|-------------|
| `value` | `-` | Value to test for plain object characteristics |

## Returns

Type-safe boolean indicating whether the value is a plain object
...
<WingletPlayground
  ...
  code={`import { isPlainObject } from '@winglet/common-utils';
