...
```typescript
const isFalsy: <T>(value: T) => value is Extract<T, Falsy>
```
...
|------|------|-------------|
| `value` | `-` | Value to test for falsy characteristics |

## Returns

Type-safe boolean indicating whether the value is falsy
...
<WingletPlayground
  ...
  code={`import { isFalsy } from '@winglet/common-utils';
