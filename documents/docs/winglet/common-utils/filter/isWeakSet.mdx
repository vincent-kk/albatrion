...
```typescript
const isWeakSet: <T extends WeakSet<any>>(value: unknown) => value is T
```
...
|------|------|-------------|
| `value` | `-` | Value to test for WeakSet type |

## Returns

Type-safe boolean indicating whether the value is a WeakSet
...
const userData = { name: 'John', age: 30, email: 'john
```
...
<WingletPlayground
  ...
  code={`import { isWeakSet } from '@winglet/common-utils';
