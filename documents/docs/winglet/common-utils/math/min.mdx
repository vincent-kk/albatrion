...
```typescript
const min: (numbers: readonly number[]) => number
```
...
|------|------|-------------|
| `numbers` | `-` | Array of numbers to find minimum from (readonly) |

## Returns

Minimum value from the array, or Infinity if array is empty
...
<WingletPlayground
  ...
  code={`import { min } from '@winglet/common-utils';
