---
title: 'primitiveArrayEqual'
sidebar_label: 'primitiveArrayEqual'
---
...
```typescript
const primitiveArrayEqual: (base: unknown[], target: unknown[]) => boolean
```
...
|------|------|-------------|
| `base` | `-` | First array to compare |
| `target` | `-` | Second array to compare |

## Returns

`true` if arrays have the same length and all elements are strictly equal, `false` otherwise
...
<WingletPlayground
  ...
  code={`import { primitiveArrayEqual } from '@winglet/common-utils';
