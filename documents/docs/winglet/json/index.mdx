...
npx -p @winglet/json claude-sync --scope=user
```

The CLI is a thin stub: it reads `@winglet/json`'s own `package.json`, resolves `claude.assetPath` (`docs/claude`), and copies the tree into the selected `.claude` directory with hash-based change detection (skip unchanged, warn on divergence, require `--force` to overwrite).
