...
function row(r: BenchResult): string {
  ...
  return `  ${r.name.padEnd(34)}  ${mount}  ${key}  ${set}`;
}

async function main(): Promise<void> {
  const { benchAdapter } = await import('../benchmarks/competitors/harness');
  const { canardAdapter, canardVirtualizedAdapter } = await import(
    '../benchmarks/competitors/canard'
  );
  ...
  for (const size of SIZES) {
    ...
    console.log(
      `  ${'library'.padEnd(34)}  ${'mount'.padStart(13)}  ${'keystroke'.padStart(15)}  ${'setValue'.padStart(15)}`,
    );
    console.log(
      `  ${''.padEnd(34)}  ${'(ms / renders)'.padStart(13)}  ${'(ms / renders)'.padStart(15)}  ${'(ms / renders)'.padStart(15)}`,
    );
  ...
  if (process.argv.includes('--assert')) {
    const canard = all.filter((r) => r.name.startsWith('@canard/schema-form'));
