...
**The third argument of `addStyle` is inverted from how it reads.** The shipped declaration is `(styleId, cssString, compress?: boolean)`, but `true` means _"this CSS is already compressed — skip the internal pass"_, not _"please compress it"_. The implementation parameter is named `compressed`, and the README documents it as `compressed`; only the `.d.ts` says `compress`. An agent reading types alone will infer the exact opposite of the truth and pass `true` on raw CSS, which then ships uncompressed. Treat the argument as `alreadyCompressed`.
