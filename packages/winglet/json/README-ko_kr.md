...
0.16.0은 0.15.0이 이전 대소문자 혼용 이름을 위해 남겨 둔 deprecated 별칭을 제거합니다. `convertJSONPathToPointer`, `convertJSONPointerToPath`를 사용하세요. `compare()`는 이제 표준 `toJSON()` 훅으로만 값을 축소합니다.
