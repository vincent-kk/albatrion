...
- 예약 멤버 이름(`__proto__`, `constructor`, `prototype`)은 모든 깊이에서 병합 대상에서 제외한다 — `setValue`의 예약 멤버 처리와 정합.
...
- 예약 멤버 제외를 재귀의 모든 깊이에서 유지한다

### Ask first

- 예약 멤버 처리 의미론(무시)을 예외 발생이나 옵션으로 바꾸는 변경
- immutable 기본값 변경
