...
describe('convertJsonPathToPointer - 멱등성 테스트 (Idempotency)', () => {
  describe('JSON Pointer 형식 입력에 대한 멱등성', () => {
    ...
    it('JSON Pointer URI Fragment 형식(#/ 시작)을 그대로 반환해야 한다', () => {
      ...
      expect(convertJsonPathToPointer('#/users/-/profile')).toBe(
        '#/users/-/profile',
      );
    });
    ...
    it('특수 문자가 포함된 JSON Pointer를 그대로 반환해야 한다', () => {
      ...
      expect(convertJsonPathToPointer('/config/db.host/port')).toBe(
        '/config/db.host/port',
      );
    });
  });
  ...
  describe('경계 케이스 멱등성', () => {
    it('숫자로만 구성된 세그먼트를 포함한 JSON Pointer의 멱등성', () => {
      ...
      numericPointers.forEach((pointer) => {
        ...
        expect(
          convertJsonPathToPointer(convertJsonPathToPointer(pointer)),
        ).toBe(pointer);
      });
    });
  ...
  describe('성능 관련 멱등성', () => {
    it('매우 긴 JSON Pointer 경로에서도 멱등성을 보장해야 한다', () => {
      ...
      expect(
        convertJsonPathToPointer(convertJsonPathToPointer(longPointer)),
      ).toBe(longPointer);
    });
    ...
    it('많은 배열 인덱스가 있는 JSON Pointer에서도 멱등성을 보장해야 한다', () => {
      ...
      expect(convertJsonPathToPointer(arrayPointer)).toBe(arrayPointer);
      expect(
        convertJsonPathToPointer(convertJsonPathToPointer(arrayPointer)),
      ).toBe(arrayPointer);
    });
  });
