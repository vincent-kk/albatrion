...
**버전**: 0.10.0 **설명**: React 유틸리티 라이브러리 — 커스텀 훅, HOC, 유틸리티 함수 **라이선스**: MIT
...
`factory`를 초기 렌더링에서 **정확히 한 번** 실행하고 결과를 React state에 보관합니다. setter를 호출하지 않으므로 값은 절대 교체되지 않고, 컴포넌트 생애 동안 참조 identity가 하나로 유지됩니다.

```typescript
// 생애주기 관리 인스턴스 — identity가 절대 바뀌면 안 되는 경우
const contextValue = useLazyConstant(() => ({
  manager: Manager.create(options),
}));

// falsy 결과에도 정확히 1회 — null로 영구 유지, 이후 렌더에서 재탐지 안 함
...
- `useMemo`/`useMemorize`와 달리 값이 **절대 재계산되지 않음이 보장**됩니다 — React는 memo 캐시를 언제든 버릴 수 있어 `useMemo`는 힌트일 뿐입니다. 팩토리 재실행이 단순 낭비가 아니라 **정확성 버그**가 되는 경우에 사용하세요.
- `useConstant`와 달리 팩토리가 **1회 실행**됩니다(`useConstant(expr)`는 매 렌더 `expr`를 평가하고 첫 결과만 유지).
- `useTruthyConstant`와 달리 **falsy를 반환해도** 1회만 실행됩니다(`null`/`false`/`0` — 재시도 없음).

**용도**: 리소스 소유 인스턴스(매니저·옵저버·스토어), mount-frozen 설정, 안정적 context 값. StrictMode(dev)에서는 폐기되는 이중 렌더로 팩토리가 두 번 호출될 수 있으니(하나만 커밋) 팩토리는 부수효과 없이 작성하세요.
