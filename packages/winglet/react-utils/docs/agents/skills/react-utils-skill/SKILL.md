...
| File                             | Contents                                                                                                                                       |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `knowledge/hooks.md`             | All 19 hooks grouped by category (state/constant · reference · lifecycle · utility) — signatures, behavior, stale-closure notes, real patterns |
...
useLazyConstant(() => x)    // never recomputes, lazy; factory runs ONCE even if falsy (correctness guarantee)
...
| Expensive one-time computation                                              | `useTruthyConstant(() => heavy())` _(lazy factory)_                              |
| One-time value whose identity must never change (manager / store / context) | `useLazyConstant(() => create())` _(lazy; correctness guarantee, even on falsy)_ |
| Pre-computed immutable value                                                | `useConstant(value)` _(eager; not a factory)_                                    |
...
6. Memoization answers must distinguish `useConstant` (stores the value or function AS-IS), `useTruthyConstant` (lazy-calls the factory on first truthy access, re-inits on falsy), and `useLazyConstant` (lazy factory that runs exactly once — even on a falsy result — with a guaranteed-stable identity, for resource-owning instances) — confusing these is the most common user mistake.
