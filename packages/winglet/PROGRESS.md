...
### [x] Task 7.4 — sortWithReference sparse 회수 (B-3, M12 재접근) · 2026-08-17 (codex 위임 + 세션 주석 보완)

**반영**: `sortWithReference.ts` — 크기 임계 분기. `reference.length <= source.length`(dense)면 **기존 선할당 경로를 문자 그대로 유지**, 반대(sparse)면 source 멤버십 `Set` 으로 `referenceMap` 을 source 실재 항목으로 축소하고 버킷을 `Map<number, Value[]>` 지연 생성 + 등장 인덱스 숫자 정렬로 조립. 버킷 의미론(`undefined` 계약)과 중복 reference last-wins 유지. characterization 선행 — 중복 reference 케이스가 기존에 없어 수정 전 green 으로 고정 후 리팩터(+1, 19→20).

**⚠ 계획 스케치의 dense 회귀와 임계 분기 (게이트가 잡음)**: 스케치(지연 그룹 단일 경로)만 적용한 측정에서 sparse 는 3.67배였지만 **dense 가 -49.2%**(217,944→110,704 hz)로 게이트를 크게 넘었다. PLAN 에 미리 적어 둔 대응 지침대로 크기 임계 분기를 추가해 dense 를 기존 경로로 되돌렸고(+3.71% 로 회복), 분기 사유(실측 기반)를 세션 검수에서 인라인 주석으로 보완했다 — 다음 편집자가 중복 제거로 통합하면 회귀가 재발하는 지점.

**성능 게이트 (hz)**

| 시나리오        | 전(기준) | 후(codex 전후쌍) |      변화 | 세션 재확인 |
| --------------- | -------: | ---------------: | --------: | ----------: |
| sparse (3/5000) |    5,656 |       **20,594** | **+264%** |      21,083 |
| dense (100/100) |  217,944 |          226,030 |     +3.7% |     209,152 |

dense 세션 절대값 차이는 환경 부하 — dense 경로는 수정 전과 동일 코드(추가 비용은 길이 비교 1회)라 코드 차원 회귀 불가.

**검증 (세션 직접 실행, 주석 보완 후)**: common-utils **127 파일 / 1,079 테스트 통과**(기존 sortWithReference 19케이스 무수정 통과), `typecheck`·`lint`·`build` 통과. 계약 불변(리팩터)이라 릴리스 기록 대상 아님.

### [x] Task 7.5 — equals·stableEquals plain-pair fast-path (A-2·A-3) · 2026-08-17 (codex 위임)

**반영**: 두 파일 모두 `OBJECT_PROTOTYPE` 모듈 상수(JSDoc 포함) + **양쪽 `getPrototypeOf` 가 `Object.prototype` 인 쌍이면 태그 경로 전체를 생략**하고 구조 비교로 직행. `equals.ts` 는 태그+builtin 블록을 조건으로 감쌌고, `stableEquals.ts` 는 배열 블록을 visited 부기 직후로 이동한 뒤 남은 태그 경로(ARRAY_BUFFER·isView·builtin)를 같은 조건으로 감쌌다. cross-realm·null-prototype·프로퍼티 붙은 내장 객체는 조건 불일치로 기존 태그 경로 그대로(기각됐던 "own key 0 지연" 의 Date 오탐 없음).

**⚠ 계획 대비 편차 — tagged-array 예외 (codex 가 계획의 논증 허점을 보정)**: 계획은 "둘 다 배열이면 태그는 둘 다 ARRAY_TAG" 를 전제로 무조건 이동을 지시했으나, **`Symbol.toStringTag` 가 붙은 배열은 위장 태그를 반환**해 기존 코드에서는 한쪽만 tagged 면 태그 불일치로 false 였다 — 무조건 이동은 이를 구조 비교 true 로 바꾸는 동작 변화다. codex 가 배열 블록 안에 `Symbol.toStringTag in left || Symbol.toStringTag in right` 일 때만 태그 판별(+ARRAY_BUFFER 특례)을 유지하는 예외를 넣어 기존 동작을 보존했다. 일반 배열은 `in` 검사 2회만 지불(태그 2회보다 저렴). `equals` 는 배열 블록이 원래 태그 앞이라 이 케이스가 기존부터 구조 비교였고 변경 없음.

**성능 게이트 (hz)**

| 시나리오                      | 원값(회귀 전) | 회귀값 | 후(codex 중앙값) | 세션 재확인 |
| ----------------------------- | ------------: | -----: | ---------------: | ----------: |
| stableEquals equal-tree (341) |        21,656 | 20,645 |           22,900 |  **24,981** |
| stableEquals cyclic           |        22,375 | 20,775 |           22,648 |  **25,274** |
| equals fully-equal (참조)     |             — | 65,166 |           69,806 |      64,904 |

M8 이 지불한 태그 비용(-4.7~-7.1%)을 회수하고 **원값을 13~15% 초과**. equals 는 7회 반복 중앙값 최악 -2.67%(5% 이내) — 동일 코드 벤치 쌍(`50 keys, equal` vs `no omit`)이 같은 실행에서 2.66% 차이 나는 측정 바닥 노이즈 안이며, 두 시나리오 변동 상관 r=0.95 로 호스트 상태가 지배 요인. 지속적 5% 초과 감속은 재현되지 않았다.

**검증 (세션 직접 실행)**: common-utils **127 파일 / 1,079 테스트 통과**(기존 4파일 91케이스 무수정 — codex 가 SHA-256 일치도 확인), `typecheck`·`lint`·`build` 통과.

### Phase 7 완료 게이트 · 2026-08-17

common-utils·json 재빌드 후 소비 패키지 전량 회귀 없음 (equals A급 소비처 31곳 포함).

| 패키지                  | 결과                      |
| ----------------------- | ------------------------- |
| `@winglet/common-utils` | 127 파일 / **1,079 통과** |
| `@winglet/json`         | 32 파일 / **550 통과**    |
| `@winglet/react-utils`  | **183 통과**              |
| `@winglet/json-schema`  | **392 통과**              |
| `@winglet/data-loader`  | **48 통과**               |
| `@lerx/promise-modal`   | **128 통과**              |
| `@canard/schema-form`   | **3,568 통과**            |

합계 **5,948 테스트, 실패 0**. 백로그 5건의 최종 회수: difference **+549%** · applyPatch 1건 **+1,511%** · stableSerialize omit **+305%** · sortWithReference sparse **+264%** · stableEquals **원값 대비 +13~15%**. 회귀로 판정된 시나리오 0건.

## Phase 7 리뷰 대응 · 2026-08-17

antigravity 교차 리뷰(범위: 계획~7.5 커밋 6건) finding 2건 + 참고 관찰 1건.

### 확인되어 고친 것

| #   | 지적                                                                                     | 검증 결과                                                                                                                                                                                                                             | 조치                                                                                                                                                     |
| --- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | 〔High〕 source 의 금지 키가 키 개수를 맞추면 target 의 신규 유효 키 패치가 **누락**된다 | CONFIRMED — **Task 7.1 검수에서 내가 넣은 금지 키 skip 이 만든 회귀**. skip 은 "모든 source 키를 비교했다" 는 조기 종료 논증을 깨는데 카운트는 skip 전 `Object.keys` 길이였다. `difference({a:1,constructor:123},{a:1,b:2})` → red `undefined` 관찰 | `hasForbidden` 플래그 — skip 발생 시 조기 종료를 포기하고 target 순회로 진입. fail-first 후 green(`{b:2}`), roundTrip +1(4→5)                            |
| 2   | 〔Low〕 53줄 재귀 헬퍼가 공개 함수 파일에 동거 — function-boundaries §3                  | 타당 — 이 패키지 관례가 이미 sibling 분리다(`compare/compareRecursive.ts`·`mergePatch/mergePatchRecursive.ts`, `@internal` 태그)                                                                                                        | `difference/differenceRecursive.ts` 로 분리(관례 정합), `differenceObjectPatch` 는 위임만                                                                 |

### 기록으로 닫은 것

| #   | 지적                                                                                  | 판단                                                                                                                                                          |
| --- | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3   | 〔참고 관찰〕 배열 원소 move 후 같은 인덱스 하위 경로 접근이 `PATCH_PATH_INVALID_INTERMEDIATE` | 리뷰어가 베이스 커밋 대조로 **pre-existing** 확인(이번 diff 에서 handleArray·setValue 무수정, 구식 구현도 동일 실패). 배열 move 의 delete-구멍 의미론 — 보류 항목 M-1(RFC 6902 splice 정합)과 같은 묶음 |

리뷰어의 CoW 위험 시나리오 실행 검증(중복 copy/move·소유 경로와 from 겹침)은 전부 안전 판정, 위험 지점 2~5(재귀 동작 대조·tagged-array 완전성·임계 분기 경계·벤치 판정 논리)도 이상 없음 판정이었다.

**검증 (세션 직접 실행, 수정 후)**: json **32 파일 / 551 테스트 통과**, `typecheck`·`lint`·`build` 통과, 벤치 no-arrays 16,601 hz(compare 15,190 의 1.09배 — 게이트 유지).
