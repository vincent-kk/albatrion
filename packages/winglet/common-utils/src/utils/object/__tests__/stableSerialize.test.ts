...
describe('stableSerialize', () => {
  ...
  it('should stableSerialize the object with circular reference', () => {
    ...
    expect(stableSerialize(directCircular)).toMatch(/{self:\d+@}/);
    expect(stableSerialize(objA)).toMatch(/{child:{parent:\d+@}}/);
    expect(stableSerialize(obj1)).toMatch(/{next:{next:{next:\d+@}}}/);
    expect(stableSerialize(arrayCircular)).toMatch(/\[{array:\d+@}\]/);
    expect(stableSerialize(complex)).toMatch(
      /%name:복잡한 객체|data:%parent:\d+@|items:#%ref:\d+@|,||/,
    );
    expect(stableSerialize(map)).toMatch(/\d+@/);
  });
});
