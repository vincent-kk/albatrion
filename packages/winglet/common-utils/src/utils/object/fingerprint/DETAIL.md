...
빠른 객체 키 생성이 목적입니다. parse 보장은 요구하지 않습니다. 기존 stableSerialize의 직접 문자열 생성과 cache hit 경로를 기반으로 구현합니다. graph wire나 encoder에 의존하지 않으며, 성능 측정과 미달 영역은 API 구현 완료와 구분해 보고합니다.

## API Contracts

세 함수는 각각 다른 목적의 키 생성기입니다. createFingerprint는 serializeObject 기반의 빠른 키 생성이며 top-level 속성만 omit하고 중첩 값에는 native JSON을 사용합니다. 속성 삽입 순서에 민감하고 native JSON의 손실·예외·toJSON 동작을 따릅니다. createSortedFingerprint는 full-sorted 기반의 경로 flatten 정렬이며 재귀 omit과 순환 marker를 제공합니다. 두 경로는 기존처럼 delimiter·타입·빈 컨테이너 구분이 모호할 수 있으며 Map/Set payload를 비교하지 않습니다.

createSafeFingerprint는 stableSerialize 기반의 정렬·순환 처리 키입니다. plain object/array의 내용을 비교하고 Date/RegExp는 값을, Map/Set/함수/사용자 인스턴스/symbol 값은 참조 identity를 비교합니다. Map/Set 내부 mutation은 키를 바꾸지 않습니다. standalone 함수의 opaque identity 범위는 module 수명이며 factory는 독립 범위입니다. factory 간 또는 프로세스 간 opaque 키 비교는 보장하지 않습니다. safe는 순환 종료와 타입/문자열 경계 구분의 의미이며 암호학적 hash나 복원 형식이 아닙니다.
...
- 일반 입력은 기존 serializeObject 출력과 같고 top-level omit과 native JSON 예외를 따릅니다.
- undefined 등 native JSON이 문자열을 만들지 않는 root에서도 키는 문자열입니다.

### fingerprint-sorted — 정렬 키

- 기존 full-sorted 출력·경로 순서·순환 marker를 유지하고 재귀 omit을 제공합니다.
...
- legacy 기반 후보의 더 약한 구분은 테스트와 결과에 명시합니다.

### performance-replacement — 대체 성능

- 각 기존 API가 담당한 workload에서 대응 경로의 속도를 비교합니다.
