...
type SchedulerFunctions<Id = any> = {
   ...
   */
  scheduleMacrotaskSafe: Fn<[callback: Fn], Id>;
  /**
   * Function to cancel scheduled macrotask
   * @param id - ID returned by scheduleMacrotask function
   */
  cancelMacrotaskSafe: Fn<[id: Id]>;
};

/**
 * Determines and returns the optimal macrotask scheduler for the current JavaScript environment.
 *
 * Uses `setImmediate` when available (Node.js) or `setTimeout` as a fallback (browsers) to ensure
 * safe rendering in browsers. Both APIs guarantee that the callback executes after the current
 * JavaScript execution context completes and after all microtasks, allowing the browser to perform
 * rendering updates between scheduled tasks. This prevents blocking the main thread and ensures
 * smooth UI updates.
 *
 ...
 *   // - Allows browser rendering between tasks
 *   // - Optimal for non-blocking operations
 * } else {
 *   // Uses setTimeout/clearTimeout (browser-safe fallback)
 *   // - Ensures safe rendering between tasks
 *   // - Prevents UI blocking
 *   // - Standard browser API for deferred execution
 * }
 * ```
 *
 * @remarks
 ...
 * - **Node.js (setImmediate)**: Executes in the next iteration of the event loop
 * - **Browsers (setTimeout)**: Minimum 4ms delay ensures rendering opportunity
 * - **Both**: Guarantee safe rendering by yielding control to the browser
 *
 * **Rendering Benefits:**
 * - **Frame Updates**: Browser can update the display between scheduled tasks
 * - **User Input**: Allows processing of user interactions between tasks
...
const getScheduleMacrotask = (): SchedulerFunctions => {
  ...
  return {
    scheduleMacrotaskSafe: globalThis.setTimeout.bind(globalThis),
    cancelMacrotaskSafe: globalThis.clearTimeout.bind(globalThis),
  } as const;
};
 ...
 */
export const scheduleMacrotaskSafe = scheduler.scheduleMacrotaskSafe;
 ...
 */
export const cancelMacrotaskSafe = scheduler.cancelMacrotaskSafe;
 ...
 */
export const scheduleCancelableMacrotaskSafe = (callback: Fn): Fn => {
  let canceled = false;
  const id = scheduleMacrotaskSafe(() => {
    if (!canceled) callback();
  });
  return () => {
    canceled = true;
    cancelMacrotaskSafe(id);
  };
};
