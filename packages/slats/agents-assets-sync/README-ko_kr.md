...
| `--scope=project`     | root marker(`.git`, `pnpm-workspace.yaml`, lockfile)를 가진 가장 가까운 조상의 `.claude`. 없으면 가장 가까운 조상 `.claude`, 그것도 없으면 `<cwd>/.claude`.                                                                                                                                                                                                                                                              |
...
`--scope=project` 의 경우 대상 `.claude` 디렉토리는 `process.cwd()` 에서 위로 올라가며 먼저 root marker(`.git`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`, `yarn.lock`, `package-lock.json`, `npm-shrinkwrap.json`, `bun.lock`, `bun.lockb`)를 가진 가장 가까운 조상을 찾고, 없을 때에만 가장 가까운 기존 `.claude` 조상을 찾아 해석됩니다. workspace 멤버 안에 생긴 `.claude` 가 프로젝트 루트로 오인되지 않습니다. 홈 디렉토리나 그 조상이 가진 marker 는 무시합니다. 자동 탐지된 경우 CLI 가 `(auto-located)` 로 로그에 표시합니다.
