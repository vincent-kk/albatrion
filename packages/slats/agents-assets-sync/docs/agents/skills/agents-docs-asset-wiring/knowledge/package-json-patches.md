...
dispatcher will exit 2 with a clear error, and `agents-build-hashes`
...
Guarantees `dist/agents-hashes.json` is regenerated before publish.
...
- The monorepo build chain still resolves `.bin/agents-build-hashes`
  from `devDependencies` at `yarn install` time — yarn workspaces
  link devDeps and deps identically for workspace-local builds.
- End users invoke the engine via `npx -p @slats/agents-assets-sync
  inject-agents-settings --package=<THIS>`, which fetches the engine
