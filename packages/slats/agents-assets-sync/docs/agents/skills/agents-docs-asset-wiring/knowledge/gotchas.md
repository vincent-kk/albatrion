...
constraint of the `@slats/agents-assets-sync` engine.

---

## The engine is the only CLI surface

`inject-agents-settings` lives in one place: the engine package. No
...
2. The monorepo build chain still resolves `.bin/agents-build-hashes`
   from `devDependencies` at `yarn install` time — yarn workspaces
   link devDeps and deps identically for workspace-local builds.
3. End users never rely on a hoisted `inject-agents-settings` bin.
   The canonical invocation is `npx -p @slats/agents-assets-sync
   inject-agents-settings --package=<THIS>`, which fetches the
...
The engine's `bin/inject-agents-settings.mjs` and
