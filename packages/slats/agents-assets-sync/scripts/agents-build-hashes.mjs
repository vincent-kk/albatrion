#!/usr/bin/env node
// Standalone CLI for `agents-build-hashes`.
//
...
// own parsed values.
//
// Convention: `pkg.agents?.assetPath` with a fallback of `'agents'`.
...
try {
  ...
  const assetPath =
    typeof pkg.agents?.assetPath === 'string' && pkg.agents.assetPath.length > 0
      ? pkg.agents.assetPath
      : 'agents';
