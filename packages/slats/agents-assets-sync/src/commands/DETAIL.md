...
- 플래그: `--package <name...>`, `--agent <type...>`, `--scope <user|project>`, `--asset <kind...>`, `--asset-path <path>`, `--dry-run`, `--force`, `--yes`, `--no-interactive`, `--root`, `--json`.
...
- `dist/agents-hashes.json` 이 없으면 plain 경로는 경고하고 건너뛰며, Ink 경로는 `useInjectSession` 을 통해 실패한 plan step 으로 표면화한다. `--asset-path` 로 온 target 은 이 판정을 받지 않는다 — 매니페스트를 애초에 읽지 않기 때문이다.
...
- `DefaultFlags` (재수출):
  - `package?: string[]`, `agent?: string[]`, `scope?: string`, `asset?: string[]`, `assetPath?: string`, `dryRun?: boolean`, `force?: boolean`, `yes?: boolean`, `interactive?: boolean`, `root?: string`, `json?: boolean`
