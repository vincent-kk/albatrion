...
2026-04-26 — `resolvePackage` accepts `originCwd` for cwd-first resolve so npx invocations see the host project's node_modules; commander program name derived from argv[1] basename so the bin alias `agents-assets-sync` self-identifies in help output.
