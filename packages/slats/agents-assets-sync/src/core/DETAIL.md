...
- A target's source hashes come from one of two places, and `resolveHashManifest` picks by `hashSource`: the stored `dist/agents-hashes.json`, or the asset directory hashed at run time. Which one a target gets is the dispatcher's call — `--asset-path` always takes the directory, and a declared asset path takes it when no manifest was built.
