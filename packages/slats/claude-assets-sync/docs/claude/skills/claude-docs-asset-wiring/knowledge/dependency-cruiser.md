...
The post-v0.3.0 layout removes the bin stub entirely, so the former `src-no-bin` and `src-no-claude-assets-sync` rules are no longer load-bearing — the consumer no longer imports any of that from anywhere. The one remaining invariant is `src/**` must not import from `docs/**`.
