...
| T-9 | **루트 `onChange`와 OnChange 검증은 매크로태스크로 디바운스된다** | `afterMicrotask`(이름과 달리 취소 뒤 재예약하는 **매크로태스크**, `AbstractNode/utils/afterMicrotask/afterMicrotask.ts:18-28`)가 `__handleChange__`를 감싼다(`AbstractNode.ts:1219-1223`) — `validate()`와 루트 `onChange`가 함께 묶인다 | 키 입력마다 소비자 콜백과 검증기가 돈다 | 4라운드 F22: 루트 `onChange`는 파동마다 한 번. 디바운스는 Form 옵션으로 두며 기본값은 소유자 결정 D-10(`reviews/round-4.md` §4). 검증은 revision 스탬프로 비동기 | `validation.*.render.test.tsx`, 루트 `onChange` 호출 횟수 단언 |
