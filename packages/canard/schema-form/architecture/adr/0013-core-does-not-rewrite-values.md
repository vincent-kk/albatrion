...
| 사건 | 종류 | 자식 원본 |
| ---- | ---- | -------- |
| 리프 입력, `setValue(V, SetValueOption.Merge)` | 부분 쓰기 | 지정한 것만 바뀐다 |
...
자동 쓰기는 다섯이다 — 채움, `controls.derived`, `controls.injectTo`, `controls.unsetValue`, 나감의 비움. 작성자가 선언하거나 호출자가 Form 속성으로 켠 것이고, 억제 비트는 그 호출이 일으킨 다섯을 모두 끈다(아래).
