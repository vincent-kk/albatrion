...
**상태 칸의 쓰기는 원본 쓰기가 아니다**(R15, ADR 0007). `setState`형 쓰기(`dirty`·`touched`)와 명령 시그널은 원본을 바꾸지 않으므로 정착의 입력도, §5의 진입도, 예약 층의 자동 쓰기(채움·`controls.derived`·`controls.injectTo`·`controls.unsetValue`·나감의 비움)도 아니다. 예약 층의 `controls.resetInteraction`(옛 `&pristine`)은 원본이 아니라 상태 칸을 초기화하며, 그 판정은 정착의 **커밋**에서 한다(`03-mental-model.md` §4). 그래서 통지보다 앞선다.
