...
- 2026-09-24 — 16라운드 답 6: 게이트(§4)에 '옛 판보다 느린 항목은 이유를 적고 Vincent가 받아들여야 병합한다'와 느린 것의 조건(통제 가능, 일정 수준 안)을 더하고, '일정 수준'의 형태와 수치를 미결에 더했다.
...
새 구현의 어떤 단계도 예산을 넘는 회귀를 안고 병합하지 않는다. 기존의 통계적 게이트(`guard:check`)를 쓴다. 옛 판보다 느린 항목은 이유를 적고 Vincent가 받아들여야 병합한다. 느린 것(대표적으로 `if`-`then`으로 스키마를 제한 없이 넓히는 문법)은 통제 가능하고 일정 수준 안이어야 한다(16라운드 답 6으로 확정, 09 §6.1).
