 ...
 *
 * v4e (round 8; loop-v4d.mjs is untouched) — execution probes for D-11′,
...
function derive(root, sid, dryRun) {
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    if (edgeGated(rule) && fires(rule, e) === false) continue;
...
function resolveDefault(host, child) {
  ...
  for (let i = 0; i < by.length; i++) {
    ...
    if (defaults !== null && Object.hasOwn(defaults, child.name)) {
...
function reconcileDerive(root, sid, dryRun) {
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    if (edgeGated(rule) && fires(rule, e) === false) {
