 ...
 * event system (§B). loop-v4b = loop-v4 with ONE change (round-5 C-1/C-7,
 * D-8 applied to select-guard unions): a host whose raw has no key selects
 * NO branch (`selection = NO_SELECTION`); the F6 rule ("값의 키 가운데 가장
 * 많이 선언한 분기, 동점이면 앞") runs only when the raw has at least one
 * key. An explicit `select()` or a loaded value activates a branch; its
 * defaults then inject as an OFF->ON transition (A2). Discriminated unions
 * (guard fragments) are untouched. Derived from loop-v3.mjs; the
...
/** `selection` value of a select-guard host with no branch active (D-8). */
 ...
 *   iff `host.selection === i` (select-guard union; `NO_SELECTION` turns
 *   every select fragment off); `declares` names the
...
/**
 * Initial selection of a select-guard host on a full replacement (A7 + D-8):
 * a raw with no key at all selects nothing; otherwise "값 키를 가장 많이
 * 선언한 분기, 동점이면 앞의 분기" (F6) — a key that no branch declares
 * (extra or unconditional child) still counts as "has a raw", so the tie
 * rule picks the first branch (REPORT-v4b.txt lists this as undefined).
 */
function initialSelection(host, value) {
  ...
  if (hasRaw === false) return NO_SELECTION;
  let best = 0;
  let bestCount = -1;
