...
describe('processOverwriteFields', () => {
  test('First-Win과 Special 필드를 제외한 모든 필드는 덮어쓰기', () => {
    ...
    const result = processOverwriteFields(base, source);

    expect(result.title).toBe('Base Title'); // 유지
    expect(result.pattern).toBe('base-pattern'); // 유지
    expect((result as any).customField1).toBe('source-custom1'); // 덮어쓰기
    expect((result as any).customField2).toBe('base-custom2'); // 유지
    expect((result as any).customField3).toBe('source-custom3'); // 추가
  });
  ...
  test('base의 모든 필드는 유지되고 source의 허용된 필드들만 추가/덮어쓰기', () => {
    ...
    expect(result.minLength).toBe(5); // 유지
    expect(result.maxLength).toBe(10); // 유지
    expect((result as any).customProp).toBe('source-value'); // 덮어쓰기
    expect((result as any).newProp).toBe('new-value'); // 추가
  });
  ...
  test('undefined 값은 추가하지 않음', () => {
    ...
    expect((result as any).customField).toBe('source-value');
  ...
  test('모든 Special 필드들이 제외됨', () => {
    ...
    expect(result.enum).toEqual(['a']); // 유지
    expect(result.const).toBe('value'); // 유지
    expect(result.required).toEqual(['field']); // 유지
    expect(result.pattern).toBe('pattern'); // 유지
    expect((result as any).minimum).toBe(0); // 유지
    expect((result as any).maximum).toBe(100); // 유지
    expect((result as any).customField).toBe('source'); // 덮어쓰기
  });
});