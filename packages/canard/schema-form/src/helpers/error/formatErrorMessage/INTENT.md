...
schema-form 런타임에서 발생하는 내부 에러(동적 함수 생성 실패, 순환 참조, 배열/조합 스키마 충돌, 가상 필드 오류 등)를 개발자가 진단할 수 있는 ASCII 박스 형식 문자열로 변환한다.
