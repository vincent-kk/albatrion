...
describe('JSON 키 유효성 및 표현식 파싱 테스트', () => {
  ...
  const transformExpression = (expression: string): { result: string; paths: string[] } => {
    const paths: string[] = [];
  ...
  // 헬퍼: 표현식 평가 테스트
  const testExpressionEvaluation = (expression: string, dependencies: any[]): any => {
  ...
  describe('🧮 표현식 파싱 및 평가', () => {
    test('특수문자 키를 포함한 표현식이 올바르게 파싱되는지', () => {
      ...
      expect(result3).toBe('(dependencies[0]) && (dependencies[1]) || (dependencies[2])');
    });
    ...
    test('유니코드 키를 포함한 표현식', () => {
      ...
      expect(result).toBe('(dependencies[0]) >= 18 && (dependencies[1]) !== null');
    ...
    test('이모지 키를 포함한 표현식', () => {
      ...
      expect(result).toBe('(dependencies[0]) === "launched" && (dependencies[1]) > 0');
  ...
  describe('🔍 실제 사용 시나리오', () => {
    ...
    test('설정 파일의 특이한 키 패턴', () => {
      ...
      // 표현식에서 사용
      const expr = '(#/app.version) === "1.0.0" && (../limits.max-connections) > 50';
  ...
  describe('⚠️ 경계 케이스 및 제한사항', () => {
    test('구조적 구분자로 인한 경로 분리', () => {
      ...
      const expr1 = '#/items[0]';  // [0]은 별도로 파싱됨
      const { paths: paths1 } = transformExpression(expr1);
      expect(paths1).toEqual(['#/items']);  // [0]은 포함되지 않음
      ...
      // 중괄호도 마찬가지
      const expr3 = '#/template{id}';  // {id}는 별도로 파싱됨
      const { paths: paths3 } = transformExpression(expr3);
      expect(paths3).toEqual(['#/template']);
