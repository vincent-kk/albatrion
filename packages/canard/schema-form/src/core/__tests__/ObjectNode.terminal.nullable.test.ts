...
describe('ObjectNode nullable functionality', () => {
  ...
  it('객체 노드가 nullable:false일 때 null 값을 처리해야 함', async () => {
    ...
    expect(value === null || value === undefined || (typeof value === 'object' && Object.keys(value).length === 0)).toBe(true);
  });
  ...
  it('nullable 객체 노드의 onChange 이벤트가 정상적으로 전파되어야 함', async () => {
    ...
    expect(mockOnChange).toHaveBeenCalledWith({ settings: { theme: 'dark', language: 'ko' } });
  ...
  it('중첩된 nullable 객체 구조가 정상적으로 동작해야 함', async () => {
    ...
    expect(value === null || (typeof value === 'object' && Object.keys(value).length === 0)).toBe(true);
  });
