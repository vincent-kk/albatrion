...
- 단일 선택은 매칭이 없을 때 -1, 다중 선택은 빈 배열을 반환합니다. 조건 컴파일 실패는 단일 선택에서 CONDITION_INDEX, 다중 선택에서 CONDITION_INDICES 오류입니다.
...
- `type`이 `null`인 항목은 조건이 있어도 선택 후보가 아닙니다. 노드가 `null`인지는 값이 정하고, 선택 결과는 객체 분기만 가리킵니다.
- 단순 등호 최적화와 일반 표현식 경로는 같은 선택 결과를 제공합니다.

## Last Updated

2026-09-16 — 단일·다중 조건 계산기의 오류 코드를 구분.
