...
- 내부(`utils/`): `createDynamicFunction`(표현식 컴파일), `getPathManager`(의존성 경로 수집), `checkComputedOptionFactory`·`getConditionIndexFactory`·`getDerivedValueFactory`·`getObservedValuesFactory`(필드 팩토리), `regex.ts`(`JSON_POINTER_PATH_REGEX`)
- 내부(패키지): `@/schema-form/types`(`JsonSchemaWithVirtual`, `JsonSchemaType`)
