...
- `children`은 자식을 가진 전략에서만 `ChildNode[]`이고, 그렇지 않으면 `null`이다. 호출자는 `null`을 "자식 없음"으로 읽어야 하며 빈 배열과 구분한다. 이 `null`은 노드 **값**의 null(nullable 배열의 null 계약, `ArrayNode/DETAIL.md`)과 무관하다.
- 두 구현체 모두 `defaultValue`가 `null`이면 `null`을 값으로 시작한다. `BranchStrategy`는 아이템 emit의 출처(`automatic`)를 배치 단위로 모아 부모에 전달하고, 변화 없다고 흡수한 밖에서의 쓰기는 null 조상이 있을 때 다시 전달한다.
...
| `push`                  | `(data?, unlimited?, option?) => Promise<number>`          | `unlimited !== true`면 `maxItems` 가드. `option`은 방출 옵션 — 폼이 스스로 추가하는 항목은 `Automatic` |
...
| `clear`                 | `() => Promise<void>`                                      | 전부 제거. 값이 `null`이면 아무것도 하지 않는다                                                        |
...
### null-parity — null 값은 두 구현체에서 동일하다

- `defaultValue: null`로 시작한 값은 두 구현체 모두 `null`이고, `clear()` 뒤에도 `null`이다.
...
- 2026-09-20 — `push`에 방출 옵션을 추가하고 null 값의 전략 간 동등성을 명문화. 이유: blank reset의 `minItems` 채움이 branch 배열에서는 부모 잠금 밖에서 emit되어 `Automatic` 표식 없이는 null 부모를 풀었다.

## Last Updated

2026-09-20 — `push` 옵션·`null-parity` 추가.
