...
- 보류 중인 쓰기를 읽은 뒤 객체에 `null`을 대입하면, 뒤따르는 보류 커밋은 읽은 값을 되살리지 않는다.
- 자동 쓰기가 draft에 남긴 값과 같은 값을 의도해서 쓰면 null 조상이 풀린다 — 같은 틱이어서 커밋이 보류 중이든, 그 커밋이 "변화 없음"으로 끝나 보류 중인 커밋이 없든 같다.
